...
/// Computes escape times for every row of the ASCII view (y = 0..<height,
/// top row first - same row range as the single-threaded asciiOutput) in
/// parallel, into a flat width*height buffer.
func computeAsciiBuffer(config: Config) -> [Int] {
...
func asciiOutput(config: Config) {
    let buffer = computeAsciiBuffer(config: config)
...
/// Computes escape times for every row of the gnuplot view in parallel.
/// Matches the single-threaded gpTextOutput's row range of y in 1...height
/// (buffer index i corresponds to y = i + 1).
func computeGpBuffer(config: Config) -> [Int] {
    let fWidth = config.ur_x - config.ll_x
    let fHeight = config.ur_y - config.ll_y
    var buffer = [Int](repeating: 0, count: config.width * config.height)
    buffer.withUnsafeMutableBufferPointer { buf in
        DispatchQueue.concurrentPerform(iterations: config.height) { i in
            let y = i + 1
            let imag = config.ur_y - Double(y) * fHeight / Double(config.height)
            let rowOffset = i * config.width
            for x in 0..<config.width {
                let real = config.ll_x + Double(x) * fWidth / Double(config.width)
                buf[rowOffset + x] = escapeTime(cr: real, ci: imag, maxIter: config.max_iter)
            }
...
func gpTextOutput(config: Config) {
    let buffer = computeGpBuffer(config: config)
    let stdout = FileHandle.standardOutput
    for i in stride(from: config.height - 1, through: 0, by: -1) {
        var rowString = ""
